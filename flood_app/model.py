...
Usage:
method1
from flood_app.model import FloodSimulator
...
class FloodSimulator:
    def __init__(self, terrain, output, model_run, infil_info, olf_info):
    ...
    def setup_grid(self):
        ...
        self.model_grid, dem_data = read_esri_ascii(
            self.terrain["grid_file"], name="topographic__elevation"
        )
        ...
        self.model_grid.add_full(
            "surface_water__depth", self.olf_info["surface_water_depth"]
        )
    ...
    def run(self):
        ...
        # duration of run (s)
        model_run_time = self.model_run["model_run_time"] * 60
        # duration of rain (s)
        storm_duration = self.model_run["storm_duration"] * 60
        ...
        for time_slice in trange(time_step, model_run_time + time_step, time_step):
            ...
            while elapsed_time < time_slice:
                ...
                if elapsed_time < storm_duration:
                    overland_flow.rainfall_intensity = self.rain_intensity / (
                        1000 * 3600
                    )
                else:
            ...
            #     plt.close(fig)
            #     fig.savefig(
            #     os.path.join(output_folder, f"flow_{time_slice}.png")
            #     )
            ...
            #     output_folder, f"infil_{time_slice}.png"))
        ...
        outlet_result["discharge_vol"] = (
            outlet_result["discharge"] * outlet_result["time_diff"]
        )
        outlet_result["cum_discharge_vol"] = outlet_result.cumsum(axis=0)[
            "discharge_vol"
        ]
